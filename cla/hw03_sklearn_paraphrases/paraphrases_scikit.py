...
def readData(trainpath, devpath, testpath):
    ...
    train_X, train_Y, vr = paraphrases_to_dataset(trainpath)
    dev_X, dev_Y, _ = paraphrases_to_dataset(devpath, vr)
    test_X, test_Y, _ = paraphrases_to_dataset(testpath, vr)
...
def paraphrases_classifier_accuracy(train_file, dev_file, test_file, verbose=False):
    ...
    lr_list = [(LogisticRegression(C=0.01, penalty="l2"), 'LogisticRegression(C=0.01, penalty="l2")'),
               ...
               (LogisticRegression(C=0.01, solver="liblinear", penalty="l1"),
               ...
               (LogisticRegression(C=1.0, solver="liblinear", penalty="l1"),
