...
class SkipGram:
    ...
    def update(self, target_id, context_id, label, learning_rate):  # TODO: Exercise 4.
        ...
        """
        ctxt_vec = self.context_word_matrix[context_id].copy()
        tgt_vec = self.target_word_matrix[target_id].copy()
        prob_pos = 1 / (1 + np.e ** - (tgt_vec@ctxt_vec)) # Replace by probability that pair belongs to positive category.
        # update context_word_matrix[context_id] using gradient and learning rate
        # update target_word_matrix[target_id] using gradient and learning rate
        self.target_word_matrix[target_id] = tgt_vec + learning_rate * (label - prob_pos) * ctxt_vec
