...
def read_word2vec_file(filename):
    ...
    with open(filename) as file:
        for line in file:
            ...
            if is_first:
                m_word_vectors = np.zeros((int(parts[0]), int(parts[1])))
                is_first = False
            else:
                m_word_vectors[row_nr] = [float(f_str) for f_str in parts[1:]]
                word_to_id[parts[0]] = row_nr
                row_nr += 1
...
def read_entity_types_file(filename, m_word_vectors, word_to_id, type_to_id = None):
    ...
    with open(filename, encoding="utf-8") as f:
        for line in f:
            ...
            if num_tokens == 0:
                avg_feature_vec = np.zeros((1, num_dims))
            else:
                avg_feature_vec = m_word_vectors[token_ids].sum(0) / num_tokens
