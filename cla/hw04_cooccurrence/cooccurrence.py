...
def cooc_dict_to_matrix(cooc_dict):
    ...
    for key1, value1 in word_to_id.items():
        for key2, value2 in word_to_id.items():
            m[value1, value2] = cooc_dict.get((key1, key2), 0)
    return m, word_to_id
...
def ppmi_weight(cooc_matrix):
    ...
    for row, col in zip(rows, cols):
        ...
        ppmi_matrix[row, col] = max(0, math.log(prc / (pr * pc)))
    return ppmi_matrix