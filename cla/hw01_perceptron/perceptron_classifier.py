...
class PerceptronClassifier:
    ...
    def update(self, instance):
        ...
        if predicted_output is True and instance.label is False:
            error = 1
        elif predicted_output is False and instance.label is True:
            error = -1
        else:
            error = 0
        do_update = error != 0
    ...
    def prediction_f_measure(self, dataset, for_label):
        ...
        for instance in dataset.instance_list:
            ...
            if instance.label == for_label:
                relevant += 1
                if prediction == instance.label:
                    tp += 1
            elif prediction != instance.label:
                fp += 1
    ...
    def features_for_class(self, is_positive_class, topn=10):
        """
        Determine the top n best features for a label (True or False).
    ...
    def save(self, filename):
        ...
        with open(filename, 'w') as model_file:
            json.dump(self.weights, model_file)
