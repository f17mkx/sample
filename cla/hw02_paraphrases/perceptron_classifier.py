...
from hw02_paraphrases.utils import dot
...
class PerceptronClassifier:
    ...
    def update(self, instance):
        ...
        if do_update:
            for feature, count in instance.feature_counts.items():
                self.weights[feature] = self.weights[feature] - error * instance.feature_counts[
                    feature]  # TODO: Replace pass with update of feature weights
        return do_update
    ...
    def prediction_f_measure(self, dataset, for_label):
        ...
        """
        # TODO
        temp = sum([1 for inst in dataset.instance_list if
                    self.prediction(inst.feature_counts) == inst.label and self.prediction(
                        inst.feature_counts) == for_label])
