...
from hw02_paraphrases.utils import DataInstance, Dataset, normalized_tokens
from hw02_paraphrases.perceptron_classifier import PerceptronClassifier
...
def token_ngrams(tokens, n):
    ...
    # return [' '.join(tokens[tokens.index(i):tokens.index(i) + n])
    #         for i in tokens if len(tokens[tokens.index(i):tokens.index(i) + n]) == n]
...
def wordpair_features(tokens1, tokens2):
    # all_combinations = ['#'.join([t1, t2]) for t1 in tokens1 for t2 in tokens2]
    # features = {pair: 1 for pair in all_combinations}

    # return {pair: 1 for pair in ['#'.join([t1, t2]) for t1 in tokens1 for t2 in tokens2]}
    return Counter(['#'.join([t1, t2]) for t1 in tokens1 for t2 in tokens2])
