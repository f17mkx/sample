...
capitals = {key: ''.join(sorted(capitals[key], key=capitals[key].get, reverse=True)[0]) for key in capitals.keys()}
# store most frequent actual form of normalized token in dictionary: normalized form to most frequent occurence

def split(word, s=0, partial_words=[]):
...
for word in freq.keys():
    ...
    if len(results) > 1:
        ...
        mean, parts = sorted(results, reverse=True)[0]
