...
def swap_half(list_a):
    ...
    """
    return concatenate_lists(list_a[len(list_a)//2:], list_a[:len(list_a)//2])
...
def long_strings(string_list, max_length):
    ...
    """
    return [True if len(e) > max_length else False for e in string_list]
...
def no_numbers(w):
    ...
    return not any([True if c.isdigit() else False for c in w])
...
def contains_substring(substring, string):
    ...
    # exp = re.compile(".*" + substring + ".*")
    return bool(re.match(r"(" + substring + ")", string))
